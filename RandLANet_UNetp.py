...
class Network:
    ...
    def inference(self, inputs, is_training):
        ...
        feature = tf.layers.dense(feature, 16, activation=None, name='fc0')
        ...
        for i in range(self.config.num_layers):
            if i == 0:
                ...
                for j in range(self.config.num_layers - i):
                    # encoding[(i, j-1)]
                    f_encoder_i_j = self.dilated_res_block(feature, inputs['xyz'][j], inputs['neigh_idx'][j], d_out[j], 'Encoder_layer_' + str(i) + str(j), is_training)
                    f_sampled_i_j = self.random_sample(f_encoder_i_j, inputs['sub_idx'][j])
            ...
            if i != 0:
                feature_list = []
                for j in range(self.config.num_layers - i):
                    # decoding[(i-1, j); (i-1, j+1)]
                    f_1 = jin[i-1][j]
                    f_2 = self.nearest_interpolation(jin[i-1][j+1], inputs['interp_idx'][j])
