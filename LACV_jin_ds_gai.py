...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('loss'):
            ...
            for i in range(self.config.num_layers - 1):
                sup_list = self.sup_list[i]
                labels = tf.reshape(self.batch_labels[i], [-1])
                for j in sup_list:
                    logits = tf.reshape(j, [-1, config.num_classes])
                    loss = self.get_loss(logits, labels, self.class_weights)
                    sup_loss = sup_loss + 0.1 * loss

            self.loss = self.get_loss(valid_logits, valid_labels, self.class_weights) + sup_loss
        ...
        for j in range(self.config.num_layers):
            if j == 0:
                ...
                for i in range(self.config.num_layers - j):
                    # encoding[(i, j-1)]
                    f_encoder_i_j, _, _ = self.dilated_res_block(feature, xyzrgb, inputs['neigh_idx'][i], d_out[i], 'Encoder_layer_' + str(j) + str(i), is_training)
            ...
            if j > 0:
                ...
                for i in range(self.config.num_layers - j):
                    ...
                    f_decoder_i_j, _, _ = self.dilated_res_block(feature, xyzrgb, inputs['neigh_idx'][i], d_out[i], 'Decoder_layer_' + str(j) + str(i), is_training)
    ...
    def dilated_res_block(self, feature, xyzrgb, neigh_idx, d_out, name, is_training):
        f_in = helper_tf_util.conv2d(feature, d_out // 2, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_pc, neigh = self.building_block(xyzrgb, f_in, neigh_idx, d_out, name + 'LFA', is_training)
    ...
    def building_block(self, xyzrgb, feature, neigh_idx, d_out, name, is_training):
        ...
        f_xyzrgb = self.loc_ie(xyzrgb, neigh_idx)

        f_xyzrgb = helper_tf_util.conv2d(f_xyzrgb, d_in, [1, 1], name + "mlp1", [1, 1], "VALID", True, is_training)
        f_neighbours = self.loc_ie(tf.squeeze(feature, axis=2), neigh_idx)
        f_concat = tf.concat([f_neighbours, f_xyzrgb], axis=-1)
        f_pc_agg, _ = self.adaptive_augmentation(f_concat, d_out // 2, name + "adap_aug_1", is_training)

        f_xyzrgb = helper_tf_util.conv2d(f_xyzrgb, d_out // 2, [1, 1], name + "mlp2", [1, 1], "VALID", True, is_training)
        f_neighbours = self.loc_ie(tf.squeeze(f_pc_agg, axis=2), neigh_idx)
        f_concat = tf.concat([f_neighbours, f_xyzrgb], axis=-1)
        f_pc_agg, f_weighted = self.adaptive_augmentation(f_concat, d_out, name + "adap_aug_2", is_training)

        # neigh = self.gather_neighbour(tf.squeeze(feature, axis=2), neigh_idx)
        # neigh = neigh + f_weighted
        return f_pc_agg, _

    def loc_ie(self, f, neigh_idx):
        tile_f = tf.tile(tf.expand_dims(f, axis=2), [1, 1, tf.shape(neigh_idx)[-1], 1])  # B, N, k, 6
        neigh_f = self.gather_neighbour(f, neigh_idx)  # B, N, k, 6
        f_info = tile_f - neigh_f
        return f_info  # B, N, k, 6+6
    ...
    @staticmethod
    def adaptive_augmentation(feature, d_out, name, is_training):
