...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            # self.Log_file = open('results/log_train_' + dataset.name + str(dataset.val_split) + self.time_stamp + '.txt', 'a')
            self.Log_file = open('results/log_train_' + dataset.name + self.time_stamp + '.txt', 'a')

        with tf.variable_scope('layers'):
            self.logits, self.feat_list, self.new_feat_list = self.inference(self.inputs, self.is_training)
        ...
        with tf.variable_scope('loss'):
            ...
            for i in range(self.config.num_layers):
                ...
                relative_dis = tf.reduce_sum(
                    tf.nn.softmax(self.feat_list[i], axis=-1) - tf.nn.softmax(centroids, axis=-1), axis=-1)
                aug_loss = aug_loss + aug_loss_weights[i] * tf.reduce_mean(tf.reduce_mean(relative_dis, axis=-1),
                                                                           axis=-1)
    ...
    def inference(self, inputs, is_training):
        ...
        sub_sampling_ratio = self.config.sub_sampling_ratio
        num_list = []
        feat_list = []
        new_feat_list = []
        ...
        f_encoder_list = []
        for i in range(self.config.num_layers):
            f_encoder_i, in_feat, new_feat = self.dilated_res_block(feature, xyzrgb, inputs['neigh_idx'][i], d_out[i], 'Encoder_layer_' + str(i), is_training)
            xyzrgb = self.random_sample(xyzrgb, inputs['sub_idx'][i], rgb=True)
            feat_list.append(tf.squeeze(in_feat, axis=2))
            new_feat_list.append(new_feat)
            f_sampled_i = self.random_sample(f_encoder_i, inputs['sub_idx'][i])
            n //= sub_sampling_ratio[i]
            num_list.append(n)
            feature = f_sampled_i
            if i == 0:
        ...
        return f_out, feat_list, new_feat_list
    ...
    def dilated_res_block(self, feature, xyz, neigh_idx, d_out, name, is_training):
        f_in = helper_tf_util.conv2d(feature, d_out, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_pc, neigh = self.building_block(xyz, f_in, neigh_idx, d_out, name + 'LFA', is_training)
        ...
        return tf.nn.leaky_relu(f_pc + shortcut), f_in, neigh

    def building_block(self, xyz, feature, neigh_idx, d_out, name, is_training):
        d_in = feature.get_shape()[-1].value
        f_xyz = self.loc_sc(xyz, neigh_idx)

        f_xyz = helper_tf_util.conv2d(f_xyz, d_in, [1, 1], name + "mlp1", [1, 1], "VALID", True, is_training)
        f_neighbours = self.loc_sc(tf.squeeze(feature, axis=2), neigh_idx)
        f_concat = tf.concat([f_neighbours, f_xyz], axis=-1)
        f_pc_agg, _ = self.att_pooling(f_concat, d_out // 2, name + "att_pooling_1", is_training)

        f_xyz = helper_tf_util.conv2d(f_xyz, d_out // 2, [1, 1], name + "mlp2", [1, 1], "VALID", True, is_training)
        f_neighbours = self.loc_sc(tf.squeeze(f_pc_agg, axis=2), neigh_idx)
        f_concat = tf.concat([f_neighbours, f_xyz], axis=-1)
        f_pc_agg, f_weighted = self.att_pooling(f_concat, d_out, name + "att_pooling_2", is_training)
    ...
    def loc_sc(self, xyzrgb, neigh_idx):
        ...
        xyzrgb_info = tile_xyzrgb - neigh_xyzrgb
        return xyzrgb_info  # B, N, k, 6+6

    def delta_nc(self, x, neigh_idx):
        ...
        delta = tile_x - neigh_x  # B, N, k, d_in
        return delta  # B, N, k, d_in

    def G_VLAD(self, feature, d_out, num_points, name, is_training):
        n_size = num_points
    ...
    @staticmethod
    def att_pooling(feature, d_out, name, is_training):
